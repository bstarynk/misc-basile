...
\begin{frame}\frametitle{Sortes de fichier}
  ...
  \begin{itemize}
  \item fichier simple (``regular file''): séquence d'octets (exemple: executable ELF, fichier textuel de configuration).
  \item periphérique de bloc (``block device''): disque ou partition
  \item periphérique d'octets (``character device''): port série, \ldots
    \item lien symbolique (``symbolic link''): contient un nom (chemin d'accès indirect)
  \item tube (``named pipe'' ou FIFO): permet la communication entre processus
    \item socquette (``socket''): permet la communication distante
...
\begin{frame}\frametitle{Type applicatif de fichier ordinaire}
  ...
  \smallskip
  Par convention le suffixe du nom d'un fichier est significatif:
  \begin{relsize}{-1}
  \begin{itemize}
