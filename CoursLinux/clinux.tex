...
\begin{frame}\frametitle{principes des shells}
  Une commande tient conceptuellement sur une longue ligne
  logique\footnote{On peut l'écrire sur plusieurs lignes physiques
  terminés par des backslashs \texttt{\textbackslash} sauf la
  dernière.\\}.
  \bigskip
  Le shell fait des expansions des ``mots'' de cette commande.
\end{frame}

%%%%
\begin{frame}\frametitle{caractères spécifiques}
  \begin{itemize}
  \item \texttt{\$} déréference une variable du shell, par exemple
    \textcolor{DarkGreen}{\texttt{echo \$PWD}}
  \item \texttt{;} sépare deux commandes exécutées en séquence :
     \textcolor{DarkGreen}{\texttt{pwd; ls}}
   \item \texttt{\textbar} pour coder un tube (cf
     ...
     \textcolor{DarkGreen}{\texttt{ls {\textbar} wc -l}}
  \end{itemize}
\end{frame}
%%%
\begin{frame}\frametitle{...}
