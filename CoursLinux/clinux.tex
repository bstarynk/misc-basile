...
%%%%
\begin{frame}\frametitle{Libertés du du logiciel libre}
...
\begin{itemize}
\item {\relsize{-1}{(liberté 0)}} la liberté d'exécuter le logiciel
  pour n'importe quel usage;
\item {\relsize{-1}{(liberté 1)}} la liberté d'étudier le
  fonctionnement d'un programme et de l'adapter à ses besoins, ce qui
  passe par l'accès aux codes sources;
\item {\relsize{-1}{(liberté 2)}} la liberté de redistribuer des
  copies du logiciel;
\item {\relsize{-1}{(liberté 3)}} la liberté de faire
  bénéficier\footnote{Il y a alors l'obligation morale voire légale de
  redistribuer le code source du logiciel amélioré} la communauté des
  versions modifiées.
\end{itemize}
\end{frame}
\end{document}
