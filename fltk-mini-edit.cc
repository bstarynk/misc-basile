...
#define DEMO_STYLE(S) do {			\
  ...
  DBGPRINTF("after Style_Comment myed_txtbuff.len %d stybuff.len %d", med->myed_txtbuff->length(), med->stybuff->length());
  DEMO_STYLE(Style_Bold);
  DEMO_STYLE(Style_Italic);
  DEMO_STYLE(Style_CodeChunk);
  DBGPRINTF("after Style_CodeChunk myed_txtbuff.len %d stybuff.len %d", med->myed_txtbuff->length(), med->stybuff->length());
#define DEMO_UNICODE_STR "§²ç°∃⁑" //∃⁑
  ...
  DBGPRINTF("do_style_demo DEMO_UNICODE_STR (%d bytes, %d utf8chars) %s"
            ...
            DEMO_UNICODE_STR,
            med->myed_txtbuff->length(), med->stybuff->length());
  med->myed_txtbuff->append(DEMO_UNICODE_STR);
  ...
  DBGPRINTF("before DEMO_STYLE Style_Unicode myed_txtbuff.len %d stybuff.len %d sb '%s'",
            med->myed_txtbuff->length(), med->stybuff->length(), sb);
  DEMO_STYLE(Style_Unicode);
  DBGPRINTF("after Style_Unicode myed_txtbuff.len %d stybuff.len %d",
            med->myed_txtbuff->length(), med->stybuff->length());
  DEMO_STYLE(Style_Errored);
  DBGPRINTF("ending do_style_demo myed_txtbuff.len %d stybuff.len %d",
            med->myed_txtbuff->length(), med->stybuff->length());
#undef DEMO_STYLE
} // end do_style_demo
...
my_rpc_compileplugin_handler(const Json::Value*pcmdjson, long cmdcount, MyAbstractCommandProcessor*cmdproc)
{
  ...
  if (my_compilation_pid < 0)
    FATALPRINTF("my_rpc_compileplugin_handler fork failed (%s)",
                strerror(errno));
  if (my_compilation_pid == 0)
    {
      // child process
...
main(int argc, char **argv)
{
  ...
  if (!my_prog_dlhandle)
    {
      FATALOUT(argv[0] << " version "
               << GITID << " failed to self-dlopen:" << dlerror());
    };
  templn = snprintf(my_tempdir, sizeof(my_tempdir), "/tmp/%s", __FILE__);
  snprintf(my_tempdir + templn - 3, sizeof(my_tempdir) - templn - 8,
           "-pid%d-git_%s", (int)getpid(), GITID);
