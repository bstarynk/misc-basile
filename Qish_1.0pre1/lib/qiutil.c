...
// prcsid $Id: qiutil.c 1.18 Thu, 23 Dec 2004 13:54:24 +0100 basile $
...
{
  if (*buf == '&') {
    if (!strncmp (buf + 1, "lt;", 3)) {
      *pend = buf + 4;
      return '<';
    } else if (!strncmp (buf + 1, "gt;", 3)) {
      *pend = buf + 4;
      return '>';
    } else if (!strncmp (buf + 1, "amp;", 4)) {
      *pend = buf + 5;
      return '&';
    } else if (!strncmp (buf + 1, "apos;", 5)) {
      *pend = buf + 6;
      return '\'';
    } else if (!strncmp (buf + 1, "asla;", 5)) {
      *pend = buf + 6;
      return '\\';
    } else if (!strncmp (buf + 1, "quot;", 5)) {
      *pend = buf + 6;
      return '\"';
    } else if (!strncmp (buf + 1, "nl;", 3)) {
      *pend = buf + 4;
      return '\n';
    } else if (!strncmp (buf + 1, "cr;", 3)) {
      *pend = buf + 4;
      return '\r';
    } else if (!strncmp (buf + 1, "bel;", 4)) {
      *pend = buf + 5;
      return '\a';
    } else if (!strncmp (buf + 1, "ff;", 3)) {
      *pend = buf + 4;
      return '\f';
    } else if (!strncmp (buf + 1, "bs;", 3)) {
      *pend = buf + 4;
      return '\b';
    } else if (!strncmp (buf + 1, "sp;", 3)) {
      *pend = buf + 4;
      return ' ';
    } else if (!strncmp (buf + 1, "tab;", 4)) {
      *pend = buf + 5;
      return '\t';
    } else if (!strncmp (buf + 1, "vtb;", 4)) {
      *pend = buf + 5;
      return '\v';
    } else if (!strncmp (buf + 1, "esc;", 4)) {
      *pend = buf + 5;
      return 27;
    } else if (!strncmp (buf + 1, "del;", 4)) {
      *pend = buf + 5;
      return 127;
    } else if (!strncmp (buf + 1, "nul;", 4)) {
      *pend = buf + 5;
      return 0;
    } else if (!strncmp (buf + 1, "nbsp;", 5)) {
      *pend = buf + 6;
      return 160;
    } else if (buf[1] == '#') {
      char *end;
...
/* eof $Id: qiutil.c 1.18 Thu, 23 Dec 2004 13:54:24 +0100 basile $ */
